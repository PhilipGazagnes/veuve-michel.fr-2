...
const { data: prestigeCuvee } = await usePrestigeCuvee()
...
<template>
  <div>
    ...
    <!-- Star card -->
    <section v-if="prestigeCuvee" class="bg-white px-6 pb-10 pt-2 md:px-14 md:pb-10 md:pt-6">
      <div class="mx-auto flex max-w-content flex-col overflow-hidden rounded-card border border-hairline-strong shadow-card md:flex-row">
        <div class="w-full md:w-[440px] md:shrink-0">
          <div class="relative aspect-[4/3] md:aspect-auto md:h-full">
            <UiPlaceholderImage :image="prestigeCuvee.image" sizes="sm:100vw md:440px" />
          </div>
        </div>
        <div class="flex flex-1 flex-col gap-3 p-7 md:p-11">
          <UiBadgePill :label="vinsStarCard.badge" />
          <h2 class="font-serif text-[27px] text-ink md:text-[38px]">{{ prestigeCuvee.name }}</h2>
          <p class="max-w-[480px] font-sans text-[15px] leading-relaxed text-body">{{ prestigeCuvee.description }}</p>
          <div class="mt-2 flex items-center justify-between gap-4 border-t border-hairline pt-4 sm:justify-start sm:gap-8">
            <span class="font-serif text-xl text-ink">{{ prestigeCuvee.price }} €</span>
            <UiButtonCta :to="`/vins/${prestigeCuvee.slug}`" :label="vinsStarCard.ctaLabel" arrow variant="solid" />
          </div>
        </div>
      </div>
    </section>
    ...
    <UiVideoLightbox v-model="isVideoOpen" :video-url="vinsVideo.videoUrl" />
  </div>
</template>
